...
export function initImport({
  addEvent, syncAndRender, escapeHtml, formatDateShort,
}) {
  addEventFn = addEvent;
...
function parseTSV(inputText) {
  // Pre-process: normalize smart quotes
  const text = normalizeQuotes(inputText);
...
function parseImportDate(inputDateStr) {
  if (!inputDateStr) return '';

  // Clean up the string
  const dateStr = inputDateStr.trim();
...
function parseProtoData(protoStr) {
  ...
  lines.forEach((line) => {
    ...
    if (statusMatch) {
      ...
      const now = new Date();
      const month = parseInt(statusMatch[1], 10);
  ...
  dataRows.forEach((row) => {
    ...
    headers.forEach((header, i) => {
      ...
      if (mapping === undefined) {
        ...
        // Proto column
        const protoNum = parseInt(mapping.replace('proto', ''), 10);
        const result = parseProtoData(value);
        if (result) {
...
function updateImportSummary() {
  ...
  // productsWithWarnings used below for UI display
...
async function confirmImport() {
  ...
  try {
    ...
    for (let i = 0; i < selectedItems.length; i += 1) {
      ...
      // Wait for the actual server POST to complete
      // (addEvent returns immediately but attaches postPromise)
      if (localEvent.postPromise) {
        // eslint-disable-next-line no-await-in-loop
        const postResult = await localEvent.postPromise;
        // eslint-disable-next-line no-console
